...
impl Artist {
    ...
    fn foward(&mut self, distance: isize) {
        match self.heading {
...
fn parse_byte(byte: u8) -> Operation {
    match byte {
        b'0' => Operation::Home,
        b'1'..=b'9' => {
            let distances = (byte - 0x30) as isize;
            Operation::Foward(distances * HEIGHT / 10)
        }
...
fn convert(ops: &Vec<Operation>) -> Vec<Command> {
    ...
    for op in ops {
        match *op {
            Operation::Foward(distance) => turtle.foward(distance),
            Operation::TurnLeft => turtle.turn_left(),
            Operation::TurnRight => turtle.turn_right(),
            Operation::Home => turtle.home(),
            Operation::Noop(byte) => eprintln!("warning: illegal byte encounting: {:?}", byte),
        };
